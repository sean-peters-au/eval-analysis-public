...
def plot_success_trend_by_messiness(
    ...
    # Create figure with 2x2 subplots (2 rows for high/low factor sum, 2 columns for time buckets)
    ...
    for col_idx, (min_time, max_time, bucket_label) in enumerate(time_buckets):
        # Process each factor sum group
        time_subset = df_messiness[df_messiness["time_bucket"] == bucket_label]
        print(
            f"Number of tasks in {bucket_label} time bucket for {test_set} test set: {len(time_subset)}"
        )
        for row_idx, (factor_filter, factor_label) in enumerate(
            [
                (lambda x: x < median_factor_sum_by_time_bucket[col_idx], "Low"),
                (lambda x: x >= median_factor_sum_by_time_bucket[col_idx], "High"),
            ]
        ):
            # Filter tasks based on factor sum
            plot_subset = time_subset[factor_filter(time_subset["factor_sum"])]
            ...
            for alias in df_runs["alias"].unique():
                ...
                # Filter runs by time bucket
                time_filtered_runs = agent_runs[
                    (agent_runs["human_minutes"] >= min_time)
                    & (agent_runs["human_minutes"] < max_time)
                ]

                weighted_success, ci_lower, ci_upper = calculate_weighted_success(
                    time_filtered_runs, plot_subset, weighting_scheme
                )
            ...
            for _, row in df_results.iterrows():
                ...
                if agent in agent_styling:
                    ...
                    # Plot error bars
                    yerr_lower = max(
                        0, float(row["weighted_success"] - row["ci_lower"])
                    )
                    yerr_upper = max(
                        0, float(row["ci_upper"] - row["weighted_success"])
                    )
                    ax.errorbar(
                        row["release_date"],
                    ...
                    ax.grid(
                        True, which="major", linestyle="-", alpha=0.25, color="black"
                    )
                    ...
                    if col_idx == 1 and row_idx == 0 and agent not in exclude_agents:
            ...
            if col_idx == 0:  # Only first column gets y-label
                ax.set_ylabel(
            ...
            factor_boundary = f"Messiness score {'>=' if factor_label == 'High' else '<'} {median_factor_sum_by_time_bucket[col_idx]:.1f}"
            ...
            ax.set_title(
                f"Task Length: {bucket_label}\n {quantile_str} {most_or_least} Messy Tasks\n({factor_boundary}, N tasks = {len(plot_subset)})",
                fontsize=ax_label_fontsize,
                pad=titlepad,
            )
    ...
    if title_source == "Combined Labels (Averaged)":
        title_source = ""
    title_str = "Model Success Rates over Time, by Task Length and Messiness"
